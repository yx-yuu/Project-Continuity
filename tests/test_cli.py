...
class ProjectContinuityTests(unittest.TestCase):
    ...
    def test_runtime_protocol_contains_only_executable_continuity_rules(self) -> None:
        ...
        self.assertIn("### 准入条件", agents)
        self.assertIn("### 放置规则", agents)
        self.assertIn("## 更新与任务结算", agents)
        self.assertIn("### 信息更新", agents)
        self.assertIn("### 按需文件生命周期", agents)
        self.assertIn("### 任务完成", agents)
        self.assertIn("### 持久化安全", agents)
        self.assertIn("## 并发约束", agents)
        for name in ("project.md", "state.md", "checkpoint.md", "current-decisions.md"):
            self.assertIn(f"`{name}`", agents)
        self.assertIn("核对其任务身份与状态", agents)
        self.assertIn("不得覆盖用户当前明确要求", agents)
        self.assertIn("已有真实权威来源的信息优先登记路由", agents)
        self.assertIn("同一信息只保留一个权威位置", agents)
        self.assertIn("项目级下一步指示阶段推进方向", agents)
        self.assertIn("任务级下一步用于恢复具体写任务", agents)
        self.assertIn("读取其完整来源", agents)
        self.assertIn("不因结构完整性创建空文件", agents)
        self.assertIn("只清理失效、被替换或重复的信息", agents)
        self.assertIn(
            "不得因内容较长、Markdown 篇幅或上下文预算而摘要、压缩、截断或删除仍然有效的信息",
            agents,
        )
        self.assertIn("允许在不损失必要细节的前提下整理结构和合并重复表述", agents)
        self.assertIn("快速启动依靠权威路由和按需读取完整来源", agents)
        self.assertIn("项目知识和项目规则写入相应权威位置，不写入本协议区块", agents)
        self.assertIn("支撑阶段判断的已核验证据", agents)
        ...
        self.assertIn("经真实来源核验且会影响后续任务的稳定项目事实", agents)
        self.assertIn("任务结算不以 checkpoint 存在为前提", agents)
        self.assertIn("详细结果保留在代码、配置、报告或其他真实权威来源中", agents)
        self.assertIn("当前有效决定必须跨任务保留", agents)
        self.assertIn("必要理由、权威依据和失效条件", agents)
        self.assertIn("当前操作、当前任务、当前项目、项目子树还是用户所有项目", agents)
        self.assertIn("符合准入条件后进入 `project.md`", agents)
        self.assertIn("进入 `state.md`，不因此自动升级为长期项目知识", agents)
        self.assertIn("当前写任务契约：进入 `checkpoint.md`", agents)
        self.assertIn("当前有效决定：进入 `current-decisions.md`", agents)
        self.assertIn("仅应用作用域覆盖当前任务且会实质影响判断的信息", agents)
        ...
        self.assertIn("另一写任务必须使用独立 worktree", agents)
        self.assertIn("普通讨论、分析、建议、搜索结果和任务输出本身不触发写入", agents)
        self.assertIn("目标状态与 Agent 核验的当前事实属于不同对象", agents)
        self.assertIn("实现完成并核验一致后，只保留统一的当前形式", agents)
        self.assertIn("修改前重新读取目标文件及相关真实来源", agents)
        self.assertIn("写入后重新读取并核验", agents)
        self.assertIn("明确的持久化请求失败时必须说明", agents)
        self.assertIn("不保存任务目标、进度、状态、约束或具体恢复步骤", agents)
        self.assertIn("一个项目级操作性下一步", agents)
        self.assertIn("一个用于恢复任务的具体下一步", agents)
        self.assertIn("已能从代码、配置等其他权威来源直接恢复时，删除对应条目", agents)
        self.assertIn("活动 checkpoint 中影响恢复的任务契约", agents)
        self.assertIn("不将未完成状态或任务级要求升级为长期知识", agents)
