...
- 更新时间：`2026-07-27T11:23:07Z`
- 当前阶段：`0.10.0 当前命名、安装体验与跨平台 CLI 修复完成`
- 当前焦点：仓库、运行时、Skill、测试和构建产物只保留 Project Continuity；命令行输出统一使用 UTF-8，README 提供可直接发送给 Codex 的完整安装提示词。
...
- CLI、wheel 校验和 release tag 校验显式使用 UTF-8 输出；subprocess 测试显式按 UTF-8 解码，跨平台路径模拟使用规范化路径。
...
- 本机 CLI 已从最终 wheel 重装为 `project-continuity 0.10.0`；Codex marketplace 指向当前工作区，插件已重装为 `0.10.0+codex.20260727112253`，状态为 installed、enabled。
